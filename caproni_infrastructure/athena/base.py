...
from caproni_infrastructure import Environment
...
class BaseAthenaBucket(s3.Bucket):
    ...
    def __init__(self, scope: core.Construct, deploy_env: Environment, **kwargs) -> None:
        ...
        self.obj_name = f's3-caproni-{self.deploy_env.value}-data-lake-athena-results'
...
class BaseAthenaWorkgroup(athena.CfnWorkGroup):
    ...
    def __init__(self, scope: core.Construct, deploy_env: Environment,
        ...
        self.obj_name = f's3-caproni-{self.deploy_env.value}-data-lake-athena-workgroup'
        super().__init__(
            scope,
            id=self.obj_name,
            name=self.obj_name,
    ...
    """ Não permite big queries, inserindo um cutoff """
    @property
